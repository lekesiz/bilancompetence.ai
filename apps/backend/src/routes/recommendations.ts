  ...
  try {
    ...
    try {
      ...
      if (!competencyIds || competencyIds.length === 0) {
        return res.status(400).json({
          status: 'error',
          message: 'No competencies found for user. Please complete your assessment first.',
        });
      }
    }
    ...
    const allJobs: any[] = [];
    const errors: string[] = [];
    const romeCodesToSearch =
      romeMatches.length > 0 ? romeMatches.slice(0, 5).map((m) => m.code) : [];

    for (const romeCode of romeCodesToSearch) {
      try {
        const searchResult = await franceTravailService.searchJobsByRomeCode(romeCode, {
          page: 1,
          minSalary,
        ...
        errors.push(`Failed to fetch jobs for category ${romeCode.toUpperCase()}`);
      }
    }
    ...
    let scoredJobs: any[] = [];
    try {
      scoredJobs = await franceTravailService.scoreJobMatches(userId, allJobs);
    ...
    logger.info(`Generated ${scoredJobs.length} recommendations for user ${userId}`);
 ...
 */
router.post('/:jobId/save', authMiddleware, async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
 ...
 */
router.get('/:userId/saved-jobs', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const savedJobs = await franceTravailService.getUserSavedJobs(userId, limit, page);
 ...
 */
router.get('/rome-codes/search', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    try {
      const results = await franceTravailService.searchRomeCodes(query, limit);
    ...
    return res.status(500).json({
      status: 'error',
      message: 'An unexpected error occurred while searching ROME codes',
    });
  }
});
 ...
 */
router.get('/rome-codes/:code', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({
      status: 'error',
      message: 'An unexpected error occurred while retrieving ROME code details',
    });
  }
});
