  ...
  for (const response of responses) {
    ...
    if (dimension && pole && dimensionScores[dimension as keyof typeof dimensionScores]) {
      ...
      if (score >= 4) {
        ...
        dimensionScores[dimension as keyof typeof dimensionScores][pole as 'E' | 'I' | 'S' | 'N' | 'T' | 'F' | 'J' | 'P'] += (score - 3);
      } else if (score <= 2) {
        // Disagree with this pole (favor the opposite)
        const oppositePole = getOppositePole(dimension, pole);
        dimensionScores[dimension as keyof typeof dimensionScores][oppositePole as 'E' | 'I' | 'S' | 'N' | 'T' | 'F' | 'J' | 'P'] += (3 - score);
      }
...
export async function calculateRIASEC(responses: RIASECResponse[]): Promise<RIASECResult> {
  ...
  const topInterests = sortedDimensions
    .slice(0, 3)
    .map((d) => d.category);
